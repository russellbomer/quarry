...
from quarry.core import load_yaml
...
def test_wizard_generates_yaml() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("quarry.wizard._prompt_text", side_effect=inputs[1:11]):
            with patch("quarry.wizard._prompt_select", side_effect=[inputs[0], inputs[8]]):
                with patch("quarry.wizard._prompt_confirm", return_value=False):
                    # Change to temp dir
                    old_cwd = os.getcwd()
                    try:
                        os.chdir(tmpdir)
