...
def _prompt_robots_override(url: str, domain: str) -> bool:
    ...
    Only called when FOUNDRY_INTERACTIVE=1 environment variable is set.
...
def get_html(
    ...
    *,
    ...
    if respect_robots and os.environ.get("FOUNDRY_IGNORE_ROBOTS") != "1":
        check_ua = ua or "Foundry/2.0 (+https://github.com/russellbomer/foundry)"
        if not _check_robots_txt(url, check_ua):
            domain = urlparse(url).netloc
            
            # Interactive mode: prompt user for override
            if os.environ.get("FOUNDRY_INTERACTIVE") == "1":
                ...
                raise PermissionError(
                    ...
                    f"  4. Set FOUNDRY_INTERACTIVE=1 to be prompted on blocks\n\n"
