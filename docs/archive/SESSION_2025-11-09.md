...
Today's work focused on **framework detection integration** and **network resilience improvements** for the Quarry wizard.
...
### Core Modules
- `quarry/http.py` - Timeout improvements (15s→30s, better retry logic)
- `quarry/wizard.py` - HTML paste fallback, error handling
- `quarry/inspector.py` - Framework detection integration, pattern boosting
- `quarry/framework_profiles.py` - Added `is_framework_pattern()` helper

### New Modules
- `quarry/detection_strategies.py` - Advanced detection (table headers, semantic HTML)
